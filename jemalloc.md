...
オブジェクトを大きさで大別する。具体的な値は、最も一般的な
64 ビット・4 KiB ページ・16 バイト quantum の既定構成だと、
最小が `8`、続いて quantum 刻みで `16, 32, 48, 64, 80, 96, 112, 128`、
以降は 2 倍ごとに 4 分割で `160, 192, 224, 256` / `320, 384, 448, 512` /
…… と続く（刻みが常に区間の 1/4 なので内部断片化は最大でも約 20%）。
small はサイズクラスごとの**スラブ**（[スラブの章](buddy-slab.md)で
学んだ、固定サイズを敷き詰めた連続領域。jemalloc はこれを slab と呼ぶ）で
管理され、ビットマップで空きスロットを追う。
ヘッダをオブジェクトごとに持たず**メタデータをデータと分離**して
別領域に置くのも特徴で、これは局所性とセキュリティの両方に効く。

> [!NOTE]
> 大別の区分と境界は **jemalloc 5.0（2017-06-13）で大きく変わった**。
> 4.x までは small（8 B〜3584 B、< 1 ページ）・large（4 KiB〜、ページ刻み）・
> huge（4 MiB〜、chunk 刻みで全スレッド共有の別構造）の **3 区分**だったが、
> 5.0 で large と huge が統合され、small（8 B〜14 KiB）と
> large（16 KiB〜 `PTRDIFF_MAX` 未満）の **2 区分**になった。
> 同時に管理単位が固定 chunk（既定 4 MiB）から可変の
> **extent（エクステント）** へ移り、後述の low-address reuse や decay は
> この extent 単位の話だ。実機の値は man ページ（jemalloc.net）か
