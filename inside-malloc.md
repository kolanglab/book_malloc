...
> 「**次の**チャンクの `prev_size` 領域」まで自分のデータに使えるので、
...
`p` への書き込み自体は静かに成功する。abort するのは、無関係に見える**次の `malloc`**——壊れた size を信じてヒープを辿った瞬間だ。スタックトレースを見ても `for` ループは出てこない。これがヒープ破壊バグのデバッグを難しくする正体である。
