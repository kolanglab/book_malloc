...
3 つの領域は説明だけだと地図が頭に入りにくい。実際にアドレスを並べてみよう。

```c
#include <stdio.h>
#include <stdlib.h>

int global_arr[4];          /* 静的領域 */
static int static_var = 42; /* 静的領域 */

int main(void) {
    int local;                     /* スタック */
    void *p = malloc(64), *q = malloc(64); /* ヒープ */
    printf("static_var (静的)   = %p\n", (void*)&static_var);
    printf("&local     (スタック) = %p\n", (void*)&local);
    printf("malloc p   (ヒープ)   = %p\n", p);
    printf("malloc q   (ヒープ)   = %p\n", q);
    printf("q - p = %ld バイト\n", (char*)q - (char*)p);
}
```

手元の Linux/x86-64 + glibc 2.39 で実行すると、たとえばこうなる（具体的なアドレスは実行のたびに変わる）。

```
static_var (静的)   = 0x5c82070b7010
&local     (スタック) = 0x7ffc670ae9c4
malloc p   (ヒープ)   = 0x5c822aece2a0
malloc q   (ヒープ)   = 0x5c822aece2f0
q - p = 80 バイト
```

静的領域とヒープはどちらも下位アドレス側（ここでは `0x5c82…`）に、スタックははるか上の `0x7ffc…` に置かれている。そして 64 バイト要求した 2 つのヒープ領域 `p`・`q` の間隔は 80 バイトだ。要求どおりの 64 ではない。アロケータが管理用の情報や切りのよい大きさのために、要求より少し大きく切り分けているからである。この「余り」が後で[断片化](#index:断片化)の話につながる。

## malloc とライブラリと OS の役割分担
...
これは口で言うより数えたほうが早い。`malloc(16)` を 10 万回呼ぶプログラムを `strace` で観察し、アロケータが OS を呼んだ回数（`brk`/`mmap`）を数えてみる。

```
$ strace -e trace=brk,mmap,munmap -c ./a.out   # malloc(16) を 10 万回
% time     seconds  usecs/call     calls    errors syscall
------ ----------- ----------- --------- --------- ----------------
  ...
```

プロセス起動時のローダによる `mmap` を除くと、10 万回の `malloc` に対してアロケータ自身が発行した `brk` はわずか 25 回ほどだった（環境やバージョンにより変わる）。`malloc` そのものはシステムコールではなく、ほとんどの呼び出しはユーザ空間だけで完結し、足りなくなったときだけ `brk` でまとめて地面を広げているのである。
...
小売業者の腕の見せどころは「`free` で返ってきた断片の再利用」だ。これが効くか効かないかで、使うメモリ量が桁で変わる。同じ手順で「埋め直すサイズ」だけを変えた 2 つのプログラムを比べてみよう。どちらも 128 バイトを 10 万個確保し、偶数番だけ解放して穴を空け、その穴を埋め直す。

```c
for (int i = 0; i < N; i++)      a[i] = malloc(128);   /* 全部確保 */
for (int i = 0; i < N; i += 2)   free(a[i]);            /* 偶数番を解放（穴あき） */
for (int i = 0; i < N; i += 2)   a[i] = malloc(SZ);     /* 穴を埋め直す */
```

`SZ` を変えて最大 RSS（実際に使った物理メモリ）を測ると、glibc 2.39 ではこうなった。

| 埋め直すサイズ `SZ` | 最大 RSS |
|---|---|
| 128（穴と同じ大きさ） | 約 16 MB |
| 4096（穴より大きい） | 約 216 MB |

同じ大きさで埋め直すと空いた穴をそのまま再利用できるので RSS は小さいまま。だが穴より大きいものを要求すると、128 バイトの穴には入らず、アロケータは地面を新しく広げるしかない。解放済みの領域が「使えるのに使えない隙間」として残る。これが[断片化](#index:断片化)だ。`malloc` と `free` の順序が任意であるという、たったそれだけの自由から、これほどの差が生まれる。
