...
合計 512 バイトが空いているのに、`malloc(512)` は `nil` を返す。0 のバディは 256、512 のバディは 768 で、どちらも使用中だからだ。これがバディの[外部断片化](#index:外部断片化)である。境界タグ法なら隣接する空きを位置に関係なく繋げられるが、バディは木上の兄弟としか結合しない——強力な自動併合の裏返しのコストである。
