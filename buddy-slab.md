...
- 要求は 2 の冪に切り上げる。$2^k$ バイトのブロックがなければ、
  $2^{k+1}$ のブロックを半分に割って作る。割った 2 つを互いの
  **バディ**（buddy; 相棒）と呼ぶ。
- 解放時、自分のバディも空きなら再結合して $2^{k+1}$ に戻す。
  これを再帰的に繰り返す。
...
b.free(y)                         # 384 と併合 → 0 と併合 → 512 と併合
p b.dump                          # => [[1024, [0]]]
```
...
ただし、空きが隣り合っていても併合できるとは限らない。256 バイトを 4 つ確保し、**互いにバディでない**先頭 0 と 512 を解放してみる。

```ruby
b = BuddyAllocator.new(10)          # 1024 バイト
a, c, d, e = b.malloc(256), b.malloc(256), b.malloc(256), b.malloc(256)
p [a, c, d, e]                      # => [0, 256, 512, 768]
b.free(a); b.free(d)                # 0 と 512 を解放（互いにバディではない）
p b.dump                            # => [[256, [0, 512]]]
p b.malloc(512)                     # => nil
```

合計 512 バイトが空いているのに、`malloc(512)` は `nil` を返す。0 のバディは 256、512 のバディは 768 で、どちらも使用中だからだ。これがバディの[外部断片化](#index:外部断片化)である。境界タグ法なら隣接する空きを位置に関係なく繋げられるが、バディは木上の兄弟としか結合しない。これは強力な自動併合の裏返しのコストである。
...
内部断片化の最悪ケースは「2 の冪をわずかに超えた要求」で起きる。代表的な要求サイズで切り上げ後のブロックと無駄を並べると、こうなる。

| 要求 | 切り上げ | ブロック | 無駄 | 無駄率 |
|---:|:---:|---:|---:|---:|
| 33 | $2^6$ | 64 | 31 | 48.4% |
| 65 | $2^7$ | 128 | 63 | 49.2% |
| 129 | $2^8$ | 256 | 127 | 49.6% |
| 200 | $2^8$ | 256 | 56 | 21.9% |
| 257 | $2^9$ | 512 | 255 | 49.8% |
| 1000 | $2^{10}$ | 1024 | 24 | 2.3% |

$2^k+1$ バイトの要求が $2^{k+1}$ ブロックを丸ごと占める形が最悪で、無駄率は $k$ を大きくするほど 50% に漸近する。逆に 1000 バイトのように 2 の冪の直下なら無駄はわずかだ。要求サイズの分布次第で痛みが大きく変わる、というのが「平均 3 割前後」の中身である。

## Linux ページアロケータの中のバディ
...
各オーダーの空きブロック数に $2^{\text{order}}$ ページ $\times$ 4 KiB を掛ければ、空きメモリ量に翻訳できる。`Normal` ゾーンを集計してみよう。

```console
$ awk '/Normal/ {tot=0; for(i=5;i<=NF;i++){o=i-5; pg=$i*(2^o); tot+=pg;
    printf "order %2d: %6d blocks = %8.1f MiB\n", o, $i, pg*4/1024}
    printf "TOTAL: %.1f MiB\n", tot*4/1024}' /proc/buddyinfo
order  0:   6276 blocks =     24.5 MiB
order  1:   8607 blocks =     67.2 MiB
order  2:   7444 blocks =    116.3 MiB
order  3:   6111 blocks =    191.0 MiB
order  4:   4228 blocks =    264.2 MiB
order  5:   2781 blocks =    347.6 MiB
order  6:   1396 blocks =    349.0 MiB
order  7:    630 blocks =    315.0 MiB
order  8:    230 blocks =    230.0 MiB
order  9:      3 blocks =      6.0 MiB
order 10:      0 blocks =      0.0 MiB
TOTAL: 1910.9 MiB
```

（値は環境により変わる。）全体で 1.8 GiB ほど空いているのに、オーダー 9 はわずか 3 ブロック、オーダー 10 にいたっては 0 ブロックしかない。低オーダーは潤沢でも高オーダーの連続領域は枯れている。これが本文で触れた「ヒュージページが取れない」断片化の正体で、空き総量は十分でも巨大な連続ブロックは作れない状態である。
...
「ページごと返却」を実際に数えてみる。20 万個のオブジェクトで eden の heap page を膨らませ、捨てて GC を回すと、ページ数が元に戻る。

```ruby
base = GC.stat(:heap_eden_pages)         # いまの heap page 数
ary  = Array.new(200_000) { Object.new } # 20 万個を敷き詰める
grow = GC.stat(:heap_eden_pages)
ary  = nil
GC.start                                 # スイープで空ページを OS へ返す
back = GC.stat(:heap_eden_pages)
p [base, grow, back]                     # => [31, 151, 31]（数は環境により変わる）
p GC::INTERNAL_CONSTANTS[:HEAP_PAGE_OBJ_LIMIT]  # => 1638
```

31 → 151 → 31。膨らんだ 120 枚ぶんは、ページ内の全スロットが空いたものから OS へ返る。1 ページに敷ける 40 バイトスロットは 1638 個（64 KiB ÷ 40 ≒ 1638）で、これがスラブで言う「1 枚の slab に並ぶオブジェクト数」にあたる。スラブの『領域単位で確保し、空になれば領域ごと回収する』設計が、そのまま GC のページ管理になっているのが見て取れる。
